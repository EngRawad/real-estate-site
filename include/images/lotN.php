...
require_once("admin/baza.php");
	 if(!$row['active']==0 ){
		if(!$fgmembersite->CheckLogin())
			...
			$query="SELECT DISTINCT * FROM category,objtype,city,region,main WHERE main.active=1 AND main.id='$_GET[id]'".$add;
...
<?  require_once("top.inc");?>
	...
	<div class="wrap_fullwidth" id='second_header'>
		<div class='center'>
			<h1 class="logo "><a href="index.php" title="CASA DE LUJO">CASA DE LUJO</a></h1>
			<ul id="nav">
   ...
   ;?>
          ...
          <? $add.=" AND category.id=main.category_id";
			...
			if($row[deal]==2)$showdeal='Сдается в аренду ';
       ...
       <div class="wrap_fullwidth"> 
    	<div class='center'>
         <span class='latest_work merguynpoqr'>Похожие результаты</span><br /><hr size="1" style="margin-top:0px;border:none; background-color:#A8231A;height:1px; color:#A8231A" />
					...
					$specc='';
                     		if($row3['is_special_offer']=='1'){
                     			 $specc='спец. предложение';?>
								 	<div class="right_float spec"><img src="images/Spec poqrnew.png"  alt="" /></div> <? } ?>
							
							 
                        <div class="content_one_third portfolio_item" style="width:">
                          <div style="width:171px; height:300px; float:right;"    class="itemm2"  onmouseover="show(1);" onmouseout="hide(1);">
                                ...
                                <table>	
                                 ...
                                 <tr>
                                    <td class="mt">
                                    		<h2><a href='lot.php?id=<?=$row3['id']?>'<?=$showdeal ?><br />
											<?=$row3[objtype]?> <br />
                                            <?=$row3[city]?></a></h2>
                        			</td>
                                 </tr>
